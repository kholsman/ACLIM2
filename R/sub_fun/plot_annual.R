...
plot_annual <-function(
  ...
  ylab      <- unique(hind%>%filter(season_var%in%season_varIN)%>%select(var,units ))
  sub_title <- unique(hind%>%filter(season_var%in%season_varIN)%>%select(var ))
  ...
  pp  <- ggplot()+
    ...
    geom_line(data=hind,
              ...
              alpha=alpha_annual,size=thinline, show.legend = FALSE)+
    geom_line(data=hind,
              aes(x=year,y=mnVal_hind+sdVal_hind),color=gg,
              alpha=alpha_annual,size=thinline*.5, show.legend = FALSE)+
    geom_line(data=hind,
              aes(x=year,y=mnVal_hind-sdVal_hind),color=gg,
              alpha=alpha_annual,size=thinline*.5, show.legend = FALSE)+
    geom_line(data=fut,
