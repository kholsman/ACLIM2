...
#' 8) ABC+HCR 8: Adjust effective B_y to mimic B_50% target
#' @param alpha default is 0.05, this is the slope of the HCR
#' @param gamma  log of the gamma parameter default is inv.logit(10); gamma decay rate value is between 0 and 1
...
#' 
    ACLIM_HCR <-function(B2B0,
      ...
      if(type%in%c(5,6)){
        ...
        gamma <- exp(log_gamma) # gamma is between 0 and 1
        if(B2B40>1.){
          #if(gamma<B2B40){
      ...
      if(type==7){
        ...
        B2B0_target <- B2B0_target*exp( -1 * omega2 * cov)
        B2B0_lim    <- B2B0_lim*exp(-1 * omega3 * cov )
        B2B40       <- B2B0/B2B0_target
        if(B2B40>1.){
          maxFabc = Flim
        }else{
      ...
      if(type%in%c(9)){
        ...
        if(B2B40>1.){
            ...
            maxFabc = Flim*exp  ( - gamma * (inv.logit(-1*cov)/.5)*(B2B40-1) )
