...
# ----------------------------------------
  ...
  if(update_base_data) 
    source(here:here("R/sub_scripts/update_base_data.R"))
  
  # load strata Area for area weighted mean temp:
  load(here("Data/in/lookup_tables","STRATA_AREA.Rdata"))
  ...
  load(here(shareddata_path,"base_data.Rdata"))
  load(here(shareddata_path,"grid_list.Rdata"))
  
  if(load_gis) 
    source(here:here("R/sub_scripts/load_maps.R"))
