...
#' THIS CODE PORTED TO QUICK START 
    ...
    srvy_indices_hind  <-  make_indices_srvyrep(simIN       = ACLIMsurveyrep,
                                          seasonsIN   = seasons, 
                                          refyrs      = deltayrs)
    ...
    # now for historical runs (CMIP6 only)
    i <- 0
    for( gcmcmip  in c("CMIP6_miroc","CMIP6_gfdl","CMIP6_cesm")){
      ...
      # area weighted  means for survey indices (annual)
      srvy_indices_hist <-  make_indices_srvyrep(simIN       = ACLIMsurveyrep,
                                            seasonsIN   = seasons, 
                                            refyrs      = deltayrs)
      ...
      # get monthly means:
      reg_indices_monthly_hist  <- reg_indices_weekly_hist%>%
        group_by(var,year,season,mo,units, long_name,basin,sim)%>%
        summarize(val = mean(val, na.rm = T))
      reg_indices_monthly_hind $type <- "monthly means"
      
      # get seasonal means:
      reg_indices_season_hist  <- reg_indices_weekly_hist%>%
        group_by(var,year,season,units, long_name,basin,sim)%>%
        summarize(val = mean(val, na.rm = T))
      ...
      reg_indices_season_hist  <- reg_indices_weekly_hist%>%
        group_by(var,year,units, long_name,basin,sim)%>%
        summarize(val = mean(val, na.rm = T))
      reg_indices_season_hist $type <- "annual means"
      ...
      for (sim in sim_listIN){
        ...
        # area weighted  means for the NEBS and SEBS separately
        load(file.path(Rdata_path,file.path(sim,paste0(reg_txt,sim,".Rdata"))))
        reg_indices_weekly_proj <- make_indices_region(simIN       = ACLIMregion,
                                                  timeblockIN = c("yr","season","mo","wk"),
                                                  seasonsIN   = seasons,
                                                  refyrs      = deltayrs,
                                                  type        = "weekly means")
        # get monthly means:
        reg_indices_monthly_proj <- reg_indices_weekly_proj%>%
          group_by(var,year,season,mo,units, long_name,basin,sim)%>%
          summarize(val = mean(val, na.rm = T))
        reg_indices_monthly_proj$type <- "monthly means"
        
        # get seasonal means:
        reg_indices_season_proj <- reg_indices_weekly_proj%>%
          group_by(var,year,season,units, long_name,basin,sim)%>%
          summarize(val = mean(val, na.rm = T))
        reg_indices_season_proj$type <- "seaonal means"
        
        # get annual means:
        reg_indices_annual_proj <- reg_indices_weekly_proj%>%
          group_by(var,year,units, long_name,basin,sim)%>%
          summarize(val = mean(val, na.rm = T))
        reg_indices_annual_proj$type <- "annual means"
