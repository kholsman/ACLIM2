...
for(v in varlist){
  ...
  dfop <- get_var_ophind(stitchDateIN = stitchDate, 
                         ...
                         plothist  = T,  # ignore the hist runs
                         removeyr1 = T)  # "Remove first year of proj ( burn in)")
  tmpdop<-ttmpdop1 <- dfop$dat%>%
    group_by(across(all_of(c(grpby,"GCM2","GCM2_scen_sim"))))%>%
    summarize_at(all_of(sumat), mean, na.rm=T)%>%
    mutate(mn_val=val_use)%>%ungroup()
  ttmpdop1%>%filter(var==v,year%in%(2022))%>%select(year, basin, var,val_use,val_delta,val_raw,GCM_scen,sim)
...
tmpd <- stitchTS(dat = tmpd, stitchDate)
#tmpd <- tmpd%>%mutate(type = "NRS indices")%>%relocate(all_of(names(NRS_vars)))%>%selec(-GCM2)
NRS_vars<- rbind(NRS_vars,tmpd)
rm(df)
rm(tmpd)    
rm(dN)
...
save(NRS_vars_op, file="Data/out/NRS_indices/NRS_vars_op.Rdata")
write.csv(NRS_vars_op, file="Data/out/NRS_indices/NRS_vars_op.csv")
...
save(NRS_vars_wide_op, file="Data/out/NRS_indices/NRS_vars_wide_op.Rdata")
write.csv(NRS_vars_wide_op, file="Data/out/NRS_indices/NRS_vars_wide_op.csv")


tt   <- NRS_vars_wide_op%>%filter(scen=="rcp45",year ==1976)
unique(tt$sim)

tt2  <- NRS_vars_op%>%filter(scen=="rcp45",year ==1976,var=="fracbelow1")
unique(tt2$sim)

tt1  <- NRS_vars_wide%>%filter(scen=="rcp45",year ==1976)
unique(tt1$sim)

tt12 <- NRS_vars%>%filter(scen=="rcp45",year ==1976,var=="fracbelow1")
unique(tt12$sim)
...
jpeg(filename = file.path("Data/out/NRS_indices/NRS_vars_byGCM.jpg"),
     width=6*sclr, height=9*sclr, units="in", res = 350)
...
jpeg(filename = file.path("Data/out/NRS_indices/NRS_vars_op_byGCM.jpg"),
     width=6*sclr, height=9*sclr, units="in", res = 350)
