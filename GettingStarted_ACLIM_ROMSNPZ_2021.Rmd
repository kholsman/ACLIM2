...
These files can be used to generate seasonal, monthly, and annual indices (like those reported in Reum et al. 2020, Holsman et al. 2020). The `Newest` folder is organized by Bering10K version, General Circulation Model (GCM) and carbon scenario, e.g. `B10K-H16_CMIP5_CESM_rcp45`. Within each folder the following subfolders are:
...
```{r monthly averages, eval=F, include=T,echo=T}
    ...
    tmp_var1    <- convert2df(ncIN = nc, type = 1, varIN = "NCaS_integrated")
    tmp_var2    <- convert2df(ncIN = nc, type = 1, varIN = "Cop_integrated")
    tmp_var3    <- convert2df(ncIN = nc, type = 1, varIN = "EupS_integrated")
    tmp_var4    <- convert2df(ncIN = nc, type = 1, varIN = "NCaS_integrated")
    
    tmp_var     <- merge(tmp_var1,tmp_var2, by=c("strata","strata_area_km2","time","basin"))
    
    head(tmp_var)
    nc_close(nc)
    getMo <- function(tIN){
      yr <- striptime(as.Date(tmp_var$t[1:4]))
      mo <- substr
    }
    tmp_var$yr     <- strptime(as.Date(tmp_var$t),format="%Y-%m-%d")$year + 1900
    tmp_var$mo     <- strptime(as.Date(tmp_var$t),format="%Y-%m-%d")$mon  + 1
    tmp_var$jday   <- strptime(as.Date(tmp_var$t),format="%Y-%m-%d")$yday + 1
