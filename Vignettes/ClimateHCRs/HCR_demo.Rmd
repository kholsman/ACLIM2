...
```{r smry, eval=T,echo=FALSE, message=FALSE, results='asis'}
library(knitr)
library(kableExtra)
library(dplyr)

hcr_table <- data.frame(
  HCR = 1:10,
  HCR_long = c(
    "ABC+HCR 1: Status quo",
    "ABC+HCR 2: Lagged recovery to estimate emergency relief financing needs",
  ...
  Goal = c(
    "This HCR is the baseline sloping harvest control rule used for groundfish in Alaska",
    "Simulations with this HCR will mimic economic-driven fishery closures and delayed recovery in order to estimate emergency relief needs.",
    "This HCR aims to enhance long-term stock resilience by adjusting B_target (as a proportion of unfished biomass)",
    "Simulations with this HCR will assess whether adjusting harvest intensity based on poor forecasted conditions—such as marine heatwaves—can accelerate stock recovery following climate or environmental disturbances.",
    "HCR 5 is designed to maximize ecosystem and spawning biomass productivity by increasing reserves, creating a buffer against environmental shocks and enhancing long-term sustainability.",
    "This HCR combines the approaches of HCR 4 and HCR 5 to address both immediate and long-term environmental impacts.",
    "This HCR provides a way to transition from qualitative risk tables to a more explicit, analytical approach for species whose productivity is known to vary with environmental conditions.",
    "This HCR adjusts the effective spawning biomass instead of the target biomass threshold, serving as a sensitivity approach to explore variability in spawning stock biomass (SSB) estimates within a given assessment year or to evaluate alternative B_target values.",
    "This HCR builds on HCR 5 by using environmental forecasts to dynamically adjust reserves, enhancing ecosystem productivity and resilience to environmental shocks.",
    "This HCR builds on HCR 5 by applying a proportional reduction in fishing mortality based on biomass levels, further enhancing stock and environmental productivity through strengthening the buffer against environmental shocks."
  )
)

# knitr::kable(hcr_table%>%select(-HCR)%>%rename(HCR=HCR_long), format = "markdown",align = "l", caption = 'Overview of ACLIM and GOACLIM 2025 HCR options.')%>%kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),full_width = F)

knitr::kable(hcr_table%>%select(-HCR)%>%rename(HCR=HCR_long), 
             format = "markdown",align = "l", 
             caption = 'Overview of ACLIM and GOACLIM 2025 HCR options.')%>%
  kableExtra::kable_styling(latex_options = "striped",bootstrap_options = c("striped", "hover"),full_width = F)%>%
  kableExtra::column_spec(1, bold = F, width = "15em") %>%
...
**Simulation goal:** `r hcr_table%>%filter(HCR==1)%>%select(Goal)%>%as.character()`.
...
**Simulation goal:** `r hcr_table%>%filter(HCR==2)%>%select(Goal)%>%as.character()`.
...
**Simulation goal:** `r hcr_table%>%filter(HCR==3)%>%select(Goal)%>%as.character()`.
...
**Simulation goal:** `r hcr_table%>%filter(HCR==4)%>%select(Goal)%>%as.character()`.
...
**Simulation goal:** `r hcr_table%>%filter(HCR==5)%>%select(Goal)%>%as.character()`.
...
**Simulation goal:** `r hcr_table%>%filter(HCR==6)%>%select(Goal)%>%as.character()`.
...
**Simulation goal:** `r hcr_table%>%filter(HCR==7)%>%select(Goal)%>%as.character()`.
...
**Simulation goal:** `r hcr_table%>%filter(HCR==8)%>%select(Goal)%>%as.character()`.
...
**Simulation goal:** `r hcr_table%>%filter(HCR==9)%>%select(Goal)%>%as.character()`.
...
**Details:** As in HCR1b (Eq.\@ref(eq:HCR1eq)), set the target to 40% of $B_{0}$, ($\alpha = 0.05$, $B_{lim} = B_{20\%}$ for amendment80 species, $B_{target} = B_{40\%}$). Shown, set $\gamma$ = `r round(exp(partxt(9,"log_gamma","a")),1)`. We’re testing whether this would result in more stable biomass levels and catches through upcoming shocks. In the example below ($x_y$) = `r plotdat_out%>%filter(type==8,Species=="Species1")%>%select(cov)%>%first()%>%as.numeric()`
...
**Simulation goal:** `r hcr_table%>%filter(HCR==10)%>%select(Goal)%>%as.character()`.
...
**Details:** As in HCR1b (Eq.\@ref(eq:HCR1eq)), set the target to 40% of $B_{0}$, ($\alpha = 0.05$, $B_{lim} = B_{20\%}$ for amendment80 species, $B_{target} = B_{40\%}$). Shown, no offset ("a") where $\gamma$ = `r round(exp(partxt(10,"log_gamma","a")),1)`, small offset ("b") $\gamma$ = `r round(exp(partxt(10,"log_gamma","b")),1)`, and ("c") large offset $\gamma$ = `r round(exp(partxt(10,"log_gamma","c")),1)`. We’re testing whether this would result in more stable biomass levels and catches through upcoming shocks. In the example below ($x_y$) = `r plotdat_out%>%filter(type==8,Species=="Species1")%>%select(cov)%>%first()%>%as.numeric()`
