...
\(B_{lim} = 0.2B_{100\%}\) (\(B_{20\%}\)) for pollock and Pacific cod .
...
to 0.3, whith \$ B\_\{low\} = B\_\{lim\} = 0.25B\_\{100\%\}\$.
...
\(B_{100\%}\) (\(\alpha = 0.05\), \(B_{lim} = B_{20\%}\),
...
have a slowly sloping F proportional to climate sensitivity (or MHW
...
\(B_{lim} = B_{20\%}\), \(B_{target} = B_{40\%}\)). Shown, for low
sensitivity stocks set \(\gamma = 0.1\), for highly sensitive stocks set
\(\gamma = 0.7\). Shown as well, category 2 MHW set the
\(\alpha = 0.23\), for large MHW set the \(\alpha = 0.41\).
...
points based on variability in SR relationships such that:
...
prep. Details: Set the target to 40\% of \(B_{100\%}\)
(\(\alpha = 0.05\), \(B_{lim} = B_{20\%}\), \(B_{target} = B_{40\%}\)).
In the example below (\(x_y\)) = 0.7 and for case 7a all \(\omega\)
values were set to 0, in case 7b \(\omega_1 = \omega_2 = \omega_3\) =
0.5, and in case 7c \(\omega_1\) = 0.7, \(\omega_2\) = 0.5, \(\omega_3\)
= .3.
...
the effective \(B_y\) and \(B_{y+1}\) is adjust downward:
...
(\(x_y\)) = 0.7 and for case 8a all \(\theta\) is set to 1, in case 8b
\(\theta\) = 0.75.
