...
biom_stats <-mn%>%left_join(se, suffix = c("_mn", "_se"),by =names(se%>%select(-num_0,-num_10prcnt,-num_50prcnt,-num_80prcnt)))
...
catch_stats <-mn%>%left_join(se, suffix = c("_mn", "_se"),by =names(se%>%select(-num_0,-num_10prcnt,-num_50prcnt,-num_80prcnt)))
...
HCRset_sub<- lapply(strsplit(HCRset,":"),"[[",1) |> unlist()

catch_stats <- catch_stats%>%mutate(HCR_num = factor(HCR_num,levels = 1:10),
                                    HCR     = factor(HCR, levels = HCRset_sub))

biom_stats  <- biom_stats%>%mutate(HCR_num = factor(HCR_num,levels = 1:10),
                                   HCR     = factor(HCR, levels = HCRset_sub))


ggplot()+
  geom_col(data=biom_stats,aes(x=HCR, y=num_50prcnt_mn,fill = factor(HCR_num,levels = 1:10)))+
  facet_grid(.~species)+
