...
Uses the strata x weekly data ('ACLIMregion') to generate strata-specific averages in order to generate the strata area-weighted averages for each week $w$ each year $y$. 
  ...
  $\bar{Y}_{w,y,k}$ was calculated for the hindcast, historical run, and projection time-series. We further used acyclic cubic regression spline (using 'mgcv::gam(...bs="cc")') across all reference years to remove artifacts (e.g. divide by 0) in the average ($\bar{Y}^{hind}_{w,k}$ and $\bar{Y}^{hist}_{w,k}$) and variance ($\sigma^{hist}_{w,k}$ and $\sigma^{hind}_{w,k}$) terms were predicted from the gam (without error; example for $\bar{Y}^{hind}_{w,k}$):

$$ \bar{Y}^{hind}_{w,k} = \mu+s(w, k = .8n)+\epsilon~~and~~\epsilon\sim N(0,\sigma)$$
Projections $\bar{Y}_{k,y,w}$ were then bias corrected using the corresponding historical and hindcast values such that:
...
```{r salmon, eval=T, include=T,echo=T, results='hide',message=FALSE}
 # loads packages, data, setup, etc.
...
```{r highres1, eval=T, include=T,echo=T, results='hide',message=FALSE}
...
``````{r highres2, eval=T, include=T,echo=T, results='hide',message=FALSE}
  #  setwd("D:/GitHub_cloud/ACLIM2")
  # loads packages, data, setup, etc.
  suppressMessages( suppressWarnings(source("R/make.R")))
...
```{r makedat1, eval=F, include=T,echo=T}
  ...
  vl <- vardef%>%filter(name%in%unique(ACLIM_weekly_hind$var))%>%
    mutate(gto = TRUE, 
           lt1 = FALSE)%>%rename(var=name)
   for(v in 1:(dim(vl)[1])){
     cat(vl$var[v],"\n")
     if((strsplit(vl$var[v],"_")[[1]][1])%in% c("aice", "fracbelow0","fracbelow1","fracbelow2" )){
       vl$gto[v] <- TRUE
       vl$lt1[v] <- TRUE
    }
     if((strsplit(vl$var[v],"_")[[1]][1])%in%c( "pH","temp",
                          "Hsbl","shflux",
                          "ssflux","vNorth","uEast")){
      vl$gto[v] <- FALSE
     }
  ...
  lt1fun <- function(x){
     if((strsplit(x,"_")[[1]][1])%in% 
        c("aice", "fracbelow0","fracbelow1","fracbelow2" )){
    out <- TRUE }else{out <- FALSE}
    return(out)
  }
    gtofun <- function(x){
      if((strsplit(x,"_")[[1]][1])%in%c( "pH","temp",
                          "Hsbl","shflux",
                          "ssflux","vNorth","uEast")){
      out <- FALSE
      }else{
    ...
    for(c in 1:length(CMIPS)){
      ...
      # norm_sub <- normlist%>%filter(var%in%varlist)
        ...
        eval(parse(text = paste0("dfut  <- ACLIM_",typeIN,"_fut")))
             ...
             dhind<-dhind%>%mutate(lt1= lt1fun(var),
                                   gto = gtofun(var))
       if(any(dhind$lt1==TRUE&dhind$mn_val>1))
                dhind$mn_val[which(dhind$lt1==TRUE&dhind$mn_val>1)]<-1
       if(any(dhind$gto==TRUE&dhind$mn_val<0))
                dhind$mn_val[which(dhind$gto==TRUE&dhind$mn_val<0)]<-0
          
             dfut<-dfut%>%mutate(lt1= lt1fun(var),
                                   gto = gtofun(var))
       if(any(dfut$lt1==TRUE&dfut$mn_val>1))
                dfut$mn_val[which(dfut$lt1==TRUE&dfut$mn_val>1)]<-1
       if(any(dfut$gto==TRUE&dfut$mn_val<0))
                dfut$mn_val[which(dfut$gto==TRUE&dfut$mn_val<0)]<-0
      ...
      # Z-score the data
       ...
       tmphind    <- suppressWarnings(dhind%>%left_join(mn)%>%
         ...
         dplyr::select(var,basin,year,mnDate,mn_val, lognorm,
                       mnVal_hind,mnhind,sdhind, sim,gcmcmip,
       ...
       dfut%>%filter(year %in%2023:2025,
         ...
         select(var,year,mn_val,sd_val,total_area_km2,val_biascorrected,
                sf_wk,mnVal_hist,mnVal_hind,val_biascorrectedyr,val_biascorrectedmo,gcmcmip)
       ...
       tmpfut    <-  suppressWarnings(dfut%>%left_join(mn)%>%
         ...
         dplyr::select(var,basin,year,mnDate,mn_val, val_biascorrected, 
                          mnVal_hind,mnhind,sdhind, sim,gcmcmip,lognorm,
                       CMIP,GCM,scen,sim_type ,units)%>%         
         dplyr::mutate(bc = "bias corrected",
                       GCM_scen = paste0(GCM,"_",scen),
      ...
      mn<-dhind%>%
         dplyr::filter(var%in%varlist,basin==plotbasin,
                       year%in%zscore_years,
                       season%in%seazn)%>%
        group_by(var,basin,season)%>%
        summarize(sdhind =sd(mn_val,na.rm=T),
                  mnhind =mean(mn_val,na.rm=T))%>%
          mutate(sfmn = -mnhind/sdhind)
        
      
       tmphind2    <- dhind%>%left_join(mn)%>%
         dplyr::filter(var%in%varlist,basin==plotbasin,
                       year%in%hind_yrs,
                       season%in%seazn)%>%
       ...
       # tmphind2$mn_val_scaled[!tmphind2$lognorm=="none"]<-tmphind2$logmn_val_scaled[!tmphind2$lognorm=="none"]
       
       tmpfut2    <- dfut%>%left_join(mn)%>%
         dplyr::filter(var%in%varlist,basin==plotbasin,
                       year%in%fut_yrs,season%in%seazn)%>%
         dplyr::mutate(var = paste0(var,"_",season))%>%
         dplyr::select(var,basin,year,mnDate,mn_val, val_biascorrected, 
                   mnVal_hind,mnhind,sdhind, sim,gcmcmip,lognorm,
                       CMIP,GCM,scen,sim_type ,units)%>%
       ...
       # ,
       #                 logmn_val_scaled =  exp((log(mn_val+log_adj)-logmnhind )/logsdhind )-log_adj)
       # tmpfut2$mn_val_scaled[!tmpfut2$lognorm=="none"]<-tmpfut2$logmn_val_scaled[!tmpfut2$lognorm=="none"]
    ...
    if(1==2){
      ...
      p
       ...
       p_bc <- ggplot(hind)+
         geom_line(aes(x=mnDate,y=mn_val,color=GCM_scen))+
         geom_line(data=fut,aes(x=mnDate,y=val_biascorrected,color=GCM_scen))+
         facet_wrap(.~var,scales="free_y")+
         theme_minimal()
      p_bc
    ...
    # -------------------------------------------
      ...
      # make bc indices for strata specific values:
        tmp <- suppressMessages(makeACLIM2_BC_Indices_new(
          ...
          BC_target = "mn_val",
          sv = NULL,
       #   sv =c("largeZoop_integrated","fracbelow2",
       #              "temp_bottom5m","temp_surface5m","pH_depthavg"),
          smoothIT  = TRUE,
      ...
      load(file.path("Data/out",CMIP_fdlr,"BC_ACLIMregion",fn))
   test<-data.frame(
fut%>%
  filter(year %in%2024,val_biascorrected>1e5,
         basin=="SEBS",mo==12,
              var=="largeZoop_integrated",
         sim=="ACLIMregion_B10K-K20P19_CMIP6_cesm_ssp585")%>%
  select(year,val_raw,val_delta,val_biascorrected,
         sf_wk,mnVal_hist,mnVal_hind,val_biascorrectedyr,val_biascorrectedmo)%>%
  mutate(dl2 = mnVal_hind + (1*(val_raw-mnVal_hist)), bc2 = mnVal_hind + (sf_wk*(val_raw-mnVal_hist))))
...
```{r annual_aclim_indicesB, eval=F, include=T,echo=T}
    # --------------------------------------
    # SETUP WORKSPACE
    # rm(list=ls())
    # setwd("D:/GitHub_cloud/ACLIM2")
    ...
    # -------------------------------------------
      ...
      # make bc indices for strata specific values:
        tmp <- suppressMessages(makeACLIM2_operational_Indices_new(
          bystrata  = TRUE,
          BC_target = "mn_val",
          sv = NULL,
          #sv = c("aice","Ben","Hsbl"),
         ...
         # make bc indices for survey rep stations
        tmp2<- suppressMessages(makeACLIM2_operational_Indices_new(
          bystrata  = FALSE,
          BC_target = "mn_val",
          sv = NULL, 
          smoothIT  = TRUE,
