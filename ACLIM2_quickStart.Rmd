...
```{r nrs, eval=T, include=T,echo=T, results='hide',message=FALSE}
 ...
 sub<- NRS_vars_op%>%filter(var%in%c("fracbelow1","fracbelow2"))
     ...
     NRS_vars_wide_op$NE_winds <- getNE_winds(vNorth=NRS_vars_wide_op$vNorth_surface5m,
                                           uEast=NRS_vars_wide_op$uEast_surface5m)
     
     # reorder to put annual indices at the end
     cc <- colnames(NRS_vars_wide_op)
     cc <- cc[-grep("annual",cc)]
     NRS_vars_wide_op<- NRS_vars_wide_op%>%relocate(all_of(cc))
...
# source("R/make.R")
rmd2md # copy and paste this into R window (won't work within markdown)
 (rmd_fl = "ACLIM2_quickStart",md_fl = "README")
