...
```{r annual_aclim_indicesA, eval=F, include=T,echo=T}
    ...
    # make bc indices for survey rep and NEBS and SEBS (weekly, monthly, seasonal, annual)
    tmp2<- suppressMessages(makeACLIM2_BC_Indices(
        bystrata  = FALSE,
        BC_target = "mn_val",
    ...
    # make bc indices for station specific values:
      tmp <- suppressMessages(makeACLIM2_BC_Indices(
        bystrata  = TRUE,
        BC_target = "mn_val",
        #sv = v,
        sv = NULL,
     ...
     if("CMIP6_K20P19_Indices"%in%ls()){                 
      ...
      gc()
        ...
        # now for strata means
      for(gcmcmipL in c("B10K-K20P19_CMIP6_miroc",
                  "B10K-K20P19_CMIP6_gfdl",
                  "B10K-K20P19_CMIP6_cesm") ){
        CMIP6_K20P19_Indices <- suppressMessages(
                        makeACLIM2_Indices(
                        ...
                        usehist      = TRUE,
                        sim_listIN   = sim_list[-grep("historical",sim_list)]))
        if(!dir.exists("data/out/K20P19_CMIP6/strata_means"))
          dir.create("data/out/K20P19_CMIP6/strata_means")
         if("CMIP6_K20P19_Indices"%in%ls()){                 
              saved <- FALSE
              saved <- save_indices(flIN = CMIP6_K20P19_Indices, 
                           subfl = "strata_means/gcmcmipL",
                           post_txt = "_mn",
                           CMIP_fdlr ="K20P19_CMIP6")
              fl <- "Data/out/CMIP6_K20P19_Indices_list.Rdata"
