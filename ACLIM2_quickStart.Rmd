...
* [00_ACLIM_shared > 02_Data > Newest_Data(use this) > 2022_05_16_unzip_and_putIN_Data_in_folder.zip](https://drive.google.com/drive/folders/11BQEfNEl9vvrN-V0LgS67XS4aLE9pNzz)
...
* [00_ACLIM_shared > 02_Data > Newest_Data(use this) > 2022_05_16_unzip_and_putIN_Data_out_folder](https://drive.google.com/drive/folders/11BQEfNEl9vvrN-V0LgS67XS4aLE9pNzz)
...
```{r makedat, eval=F, include=T,echo=T}
  ...
  # define hind and fut data files
    ...
    for(c in 1:length(CMIPS)){
      ...
      # 
      #  # rescale the data using mean of the hind if norm distributed
      # dhind$log_mn_val <- log(dhind$mn_val+log_adj)
      # dfut$log_mn_val  <- log(dfut$mn_val+log_adj)
      
       tmphind    <- suppressWarnings(dhind%>%
         dplyr::filter(var%in%varlist,basin==plotbasin,year%in%hind_yrs)%>%
        # dplyr::left_join(norm_sub,by=c("var"="var"))%>%
         dplyr::select(var,basin,year, jday,mnDate,mn_val, lognorm,
         ...
         dplyr::mutate(bc = "bias corrected",
                       ...
                       # mn_val_scaled = ifelse(lognorm,
                       #      ((log(mn_val+log_adj)-log(mnVal_hind+log_adj) )
                       #      /sqrt(log(sdVal_hind+log_adj)) )-log_adj,
                       #      (mn_val-mnVal_hind )/sqrt(sdVal_hind)  )) )
       ...
       p_raw <- ggplot(tmphind)+geom_line(aes(x=year,y=mn_val,color=scen),size=.8)+
         geom_line(data =tmpfut,aes(x=year,y=mn_val,color=scen, linetype = GCM ),size=.8)+
         facet_wrap(var~.,scales="free_y")+theme_minimal()
       p_scaled <- ggplot(tmphind)+
         geom_line(aes(x=year,y=mn_val_scaled,color=scen),size=.8)+
         geom_line(data =tmpfut,aes(x=year,y=mn_val_scaled,color=scen, linetype = GCM ),size=.8)+
         facet_wrap(var~.,scales="free_y")+theme_minimal()
      p_raw
      p_scaled
...
```{r annual_aclim_indices, eval=F, include=T,echo=T}
    ...
    # load(file.path(Rdata_path,"rm_wk_list.Rdata"))
    # load(file.path(Rdata_path,"rm_var_list.Rdata"))
    # 
    # 
