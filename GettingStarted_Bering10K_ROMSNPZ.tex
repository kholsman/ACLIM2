...
\NormalTok{    tt <-}\StringTok{ }\KeywordTok{seq}\NormalTok{(}\KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{substring}\NormalTok{(tt}\OperatorTok{$}\NormalTok{Start,}\DecValTok{1}\NormalTok{,}\DecValTok{4}\NormalTok{)),}
              \KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{substring}\NormalTok{(tt}\OperatorTok{$}\NormalTok{End,}\DecValTok{1}\NormalTok{,}\DecValTok{4}\NormalTok{)),}\DecValTok{10}\NormalTok{)}
...
\NormalTok{    time_seg   <-}\StringTok{ }\KeywordTok{list}\NormalTok{( }\StringTok{'1970-1980'}\NormalTok{ =}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{1970}\OperatorTok{:}\DecValTok{1980}\NormalTok{),}
     ...
     \CommentTok{# preview the datasets on the server:}
     \KeywordTok{tds_list_datasets}\NormalTok{(}\DataTypeTok{thredds_url =}\NormalTok{ ACLIM_data_url)}
...
\NormalTok{    endTime  }\OperatorTok{-}\StringTok{ }\NormalTok{startTime}
    ...
    \CommentTok{#       }\AlertTok{NOTE}\CommentTok{: THIS IS SLOOOOOW..~ 2 min}
\NormalTok{    startTime2 =}\StringTok{ }\KeywordTok{Sys.time}\NormalTok{()}
    \ControlFlowTok{if}\NormalTok{(}\OperatorTok{!}\KeywordTok{file.exists}\NormalTok{(}\KeywordTok{file.path}\NormalTok{(Rdata_path,fl)))\{}
      \KeywordTok{get_l2}\NormalTok{(}
        ...
        \DataTypeTok{sim_list    =}\NormalTok{ sim  )}
\NormalTok{    \}}
\NormalTok{    endTime2  =}\StringTok{ }\KeywordTok{Sys.time}\NormalTok{()}
\NormalTok{    endTime2  }\OperatorTok{-}\StringTok{ }\NormalTok{startTime2}
...
\NormalTok{    j<-}\DecValTok{0}
    \ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(time_seg))\{}
      \ControlFlowTok{if}\NormalTok{(}\KeywordTok{length}\NormalTok{( }\KeywordTok{which}\NormalTok{(}\KeywordTok{as.numeric}\NormalTok{(tmp_var}\OperatorTok{$}\NormalTok{year)}\OperatorTok{%in%}\NormalTok{time_seg[[i]] ))}\OperatorTok{>}\DecValTok{0}\NormalTok{)\{}
...
\NormalTok{    p9_hind     <-}\StringTok{ }\KeywordTok{plot_stations_basemap}\NormalTok{(}\DataTypeTok{sfIN =}\NormalTok{ L2_sf,}
                                \DataTypeTok{fillIN =} \StringTok{"mnval"}\NormalTok{,}
                                \DataTypeTok{colorIN =} \StringTok{"mnval"}\NormalTok{,}
                                \DataTypeTok{sizeIN=}\NormalTok{.}\DecValTok{6}\NormalTok{) }\OperatorTok{+}
\StringTok{      }\CommentTok{#facet_wrap(.~time_period,nrow=2,ncol=3)+}
...
\NormalTok{    p9_hind}
    ...
    \ControlFlowTok{if}\NormalTok{(update.figs)  }
      \KeywordTok{ggsave}\NormalTok{(}\DataTypeTok{file=}\KeywordTok{file.path}\NormalTok{(main,}\StringTok{"Figs/Hind_sub_grid_mn_BT_Aug1.jpg"}\NormalTok{),}\DataTypeTok{width=}\DecValTok{8}\NormalTok{,}\DataTypeTok{height=}\DecValTok{4}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
    \CommentTok{# M2_lat <- (56.87°N, -164.06°W)}
    \CommentTok{# 56.877    -164.06 xi = 99    eta= 62}
\NormalTok{    IDin       <-}\StringTok{ "_2013_M2"}
\NormalTok{    var_use    <-}\StringTok{ "_surface5m_temp"}
    
    \CommentTok{# get data from M2 data page:}
\NormalTok{      pmelM2_url <-}\StringTok{"https://www.ncei.noaa.gov/data/oceans/ncei/ocads/data/0157599/"}
\NormalTok{      yr_dat     <-}\StringTok{ "M2_164W_57N_Apr2019_May2019.csv"}
\NormalTok{      yr_dat     <-}\StringTok{ "M2_164W_57N_May2013_Sep2013.csv"}
        
    \CommentTok{# preview the datasets on the server:}
\NormalTok{      temp <-}\StringTok{ }\KeywordTok{tempfile}\NormalTok{()}
      \KeywordTok{download.file}\NormalTok{(}\KeywordTok{paste0}\NormalTok{(pmelM2_url,yr_dat),temp)}
      \CommentTok{#M2data <- read.csv(temp,skip=4,stringsAsFactors = F)}
\NormalTok{      M2data <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(temp,}\DataTypeTok{skip=}\DecValTok{0}\NormalTok{,}\DataTypeTok{stringsAsFactors =}\NormalTok{ F)}
      
      \KeywordTok{unlink}\NormalTok{(temp)}

    \CommentTok{# convert date and time to t }
\NormalTok{      M2data}\OperatorTok{$}\NormalTok{t <-}\KeywordTok{as.POSIXct}\NormalTok{(}\KeywordTok{paste0}\NormalTok{(M2data}\OperatorTok{$}\NormalTok{Date,}\StringTok{" "}\NormalTok{,M2data}\OperatorTok{$}\NormalTok{Time,}\StringTok{":00"}\NormalTok{),}\StringTok{"%m/%d/%Y %H:%M:%S"}\NormalTok{,}
                             \DataTypeTok{origin =}   \StringTok{"1900-01-01 00:00:00"}\NormalTok{,}
...
\NormalTok{    sim        <-}\StringTok{ "B10K-K20_CORECFS"} 
    ...
    \CommentTok{# Let's sample the model years as close to Aug 1 as the model timesteps run:}
    \CommentTok{#tr          <- c("-08-1 12:00:00 GMT") }
\NormalTok{    tr          <-}\StringTok{ }\KeywordTok{substring}\NormalTok{(M2data}\OperatorTok{$}\NormalTok{t,}\DecValTok{5}\NormalTok{,}\DecValTok{20}\NormalTok{)}
    \CommentTok{# the full grid is large and takes a longtime to plot, so let's subsample the grid every 4 cells}
   ...
   \CommentTok{# load data from level 2 nc files (grab a coffee, takes a few mins)}
    \ControlFlowTok{if}\NormalTok{(}\OperatorTok{!}\KeywordTok{file.exists}\NormalTok{(}\KeywordTok{file.path}\NormalTok{(Rdata_path,fl)))\{}
      \KeywordTok{get_l2}\NormalTok{(}
        \DataTypeTok{ID          =}\NormalTok{ IDin,}
        \DataTypeTok{overwrite   =}\NormalTok{ T,}
        \DataTypeTok{xi_rangeIN  =} \DecValTok{99}\NormalTok{,}
        \DataTypeTok{eta_rangeIN =} \DecValTok{62}\NormalTok{,}
        \DataTypeTok{ds_list     =}\NormalTok{ dl[}\DecValTok{2}\NormalTok{],  }\CommentTok{# must be same length as sub_varlist}
        \DataTypeTok{trIN        =}\NormalTok{ tr,}
        \DataTypeTok{yearsIN     =} \DecValTok{2013}\NormalTok{,}
        \DataTypeTok{sub_varlist =} \KeywordTok{list}\NormalTok{(}\StringTok{'Surface 5m'}\NormalTok{ =}\StringTok{ "temp"}\NormalTok{ ),  }
        \DataTypeTok{sim_list    =} \KeywordTok{c}\NormalTok{(}\StringTok{"B10K-H16_CORECFS"}\NormalTok{,}\StringTok{"B10K-K20_CORECFS"}\NormalTok{ )  )}
\NormalTok{    \}}
    
    \CommentTok{# load R data file}
     \CommentTok{# open a "region" or strata specific nc file}
\NormalTok{    sim <-}\StringTok{ "B10K-H16_CORECFS"}
\NormalTok{    fl         <-}\StringTok{ }\KeywordTok{file.path}\NormalTok{(main,Rdata_path,sim,}\StringTok{"Level2"}\NormalTok{,}
                            \KeywordTok{paste0}\NormalTok{(sim,var_use,IDin,}\StringTok{".Rdata"}\NormalTok{))}
    \KeywordTok{load}\NormalTok{(fl)   }\CommentTok{# temp}
...
\NormalTok{                       )}
     \CommentTok{# open a "region" or strata specific nc file}
\NormalTok{    sim <-}\StringTok{ "B10K-K20_CORECFS"}
\NormalTok{    fl2         <-}\StringTok{ }\KeywordTok{file.path}\NormalTok{(main,Rdata_path,sim,}\StringTok{"Level2"}\NormalTok{,}
                            \KeywordTok{paste0}\NormalTok{(sim,var_use,IDin,}\StringTok{".Rdata"}\NormalTok{))}
    \KeywordTok{load}\NormalTok{(fl2)   }\CommentTok{# temp}
    \ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\KeywordTok{dim}\NormalTok{(temp}\OperatorTok{$}\NormalTok{val)[}\DecValTok{3}\NormalTok{])}
\NormalTok{      data_long <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(data_long,}
                          \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{latitude =} \KeywordTok{as.vector}\NormalTok{(temp}\OperatorTok{$}\NormalTok{lat),}
                           \DataTypeTok{longitude =} \KeywordTok{as.vector}\NormalTok{(temp}\OperatorTok{$}\NormalTok{lon),}
                           \DataTypeTok{val =} \KeywordTok{as.vector}\NormalTok{(temp}\OperatorTok{$}\NormalTok{val[,,i]),}
...
\NormalTok{    plotM2_dat        <-}\StringTok{ }\NormalTok{M2data}\OperatorTok{%>%}\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{select}\NormalTok{(}\DataTypeTok{SST =}\NormalTok{ SST..C.,}\DataTypeTok{Date =}\NormalTok{ t)}
\NormalTok{    plotM2_dat}\OperatorTok{$}\NormalTok{sim    <-}\StringTok{ }\KeywordTok{factor}\NormalTok{(}\StringTok{"Obs"}\NormalTok{,}\DataTypeTok{levels=}\KeywordTok{c}\NormalTok{(}\StringTok{"Obs"}\NormalTok{,}\StringTok{"B10K-H16_CORECFS"}\NormalTok{,}\StringTok{"B10K-K20_CORECFS"}\NormalTok{))}
\NormalTok{    plotM2_dat        <-}\StringTok{ }\NormalTok{plotM2_dat}\OperatorTok{%>%}\KeywordTok{filter}\NormalTok{(SST}\OperatorTok{>-}\DecValTok{99}\NormalTok{)}
\NormalTok{    plotroms_dat      <-}\StringTok{ }\NormalTok{data_long}\OperatorTok{%>%}\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{select}\NormalTok{(}\DataTypeTok{SST =}\NormalTok{ val,}\DataTypeTok{Date =}\NormalTok{ time,sim)}
\NormalTok{    plotroms_dat}\OperatorTok{$}\NormalTok{sim  <-}\StringTok{ }\KeywordTok{factor}\NormalTok{(plotroms_dat}\OperatorTok{$}\NormalTok{sim,}\DataTypeTok{levels=}\KeywordTok{c}\NormalTok{(}\StringTok{"Obs"}\NormalTok{,}\StringTok{"B10K-H16_CORECFS"}\NormalTok{,}\StringTok{"B10K-K20_CORECFS"}\NormalTok{))}
\NormalTok{    plotdat           <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(plotM2_dat,plotroms_dat)}
   
\NormalTok{    p10_hind     <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(plotdat) }\OperatorTok{+}
\StringTok{      }\KeywordTok{geom_line}\NormalTok{(   }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{Date,}\DataTypeTok{y=}\NormalTok{SST,}\DataTypeTok{color=}\NormalTok{sim),}\DataTypeTok{alpha=}\NormalTok{.}\DecValTok{8}\NormalTok{)}\OperatorTok{+}
\StringTok{      }\CommentTok{# geom_smooth( aes(x = Date,y = SST,color=sim),}
\StringTok{      }\CommentTok{#             formula = y ~ x, se = T)+}
...
\StringTok{      }\KeywordTok{ylab}\NormalTok{(tmp_var}\OperatorTok{$}\NormalTok{units[}\DecValTok{1}\NormalTok{])}\OperatorTok{+}
\StringTok{      }\KeywordTok{ggtitle}\NormalTok{( }\StringTok{"Bering M2 Mooring: 2013 SST"}\NormalTok{)}\OperatorTok{+}
\StringTok{      }\KeywordTok{theme_minimal}\NormalTok{()}
...
\NormalTok{    p10_hind}
    ...
    \ControlFlowTok{if}\NormalTok{(update.figs)  }
      \KeywordTok{ggsave}\NormalTok{(}\DataTypeTok{file=}\KeywordTok{file.path}\NormalTok{(main,}\StringTok{"Figs/Hind_M2_SST.jpg"}\NormalTok{),}\DataTypeTok{width=}\DecValTok{8}\NormalTok{,}\DataTypeTok{height=}\DecValTok{4}\NormalTok{)}
...
\NormalTok{    p3}
    ...
    \ControlFlowTok{if}\NormalTok{(update.figs)  }
      \KeywordTok{ggsave}\NormalTok{(}\DataTypeTok{file=}\KeywordTok{file.path}\NormalTok{(main,}\StringTok{"Figs/mn_BT.jpg"}\NormalTok{),}\DataTypeTok{width=}\DecValTok{8}\NormalTok{,}\DataTypeTok{height=}\DecValTok{6}\NormalTok{)}
...
\NormalTok{    var_use   <-}\StringTok{ "temp_bottom5m"}
    \CommentTok{# tinker: var_use <- "Cop_integrated"}
...
\NormalTok{  p5}
  \ControlFlowTok{if}\NormalTok{(update.figs)  }
    \KeywordTok{ggsave}\NormalTok{(}\DataTypeTok{file=}\KeywordTok{file.path}\NormalTok{(main,}\StringTok{"Figs/weekly_byreg.jpg"}\NormalTok{),}\DataTypeTok{width=}\DecValTok{8}\NormalTok{,}\DataTypeTok{height=}\DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sim_set <-}\KeywordTok{c}\NormalTok{(}\StringTok{"B10K-K20_CORECFS"}\NormalTok{,}
            \StringTok{"B10K-K20P19_CMIP6_miroc_historical"}\NormalTok{,}
            \StringTok{"B10K-K20P19_CMIP6_miroc_ssp126"}\NormalTok{,}
            \StringTok{"B10K-K20P19_CMIP6_miroc_ssp585"}\NormalTok{)}
...
\NormalTok{ii<-}\DecValTok{0}
\CommentTok{# open a "region" or strata specific  file}
\ControlFlowTok{for}\NormalTok{(sim }\ControlFlowTok{in}\NormalTok{ sim_set)\{}
...
\NormalTok{  fl      <-}\StringTok{ }\KeywordTok{file.path}\NormalTok{(sim,}\KeywordTok{paste0}\NormalTok{(reg_txt,sim,}\StringTok{".Rdata"}\NormalTok{))}
  ...
  \ControlFlowTok{if}\NormalTok{(}\OperatorTok{!}\KeywordTok{file.exists}\NormalTok{(}\KeywordTok{file.path}\NormalTok{(Rdata_path,fl)))}
    \KeywordTok{get_l3}\NormalTok{(}\DataTypeTok{web_nc =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{download_nc =}\NormalTok{ F,}
...
\NormalTok{  tmp_var}\OperatorTok{$}\NormalTok{sim    <-}\StringTok{ }\NormalTok{sim}
  \ControlFlowTok{if}\NormalTok{(ii }\OperatorTok{==}\StringTok{ }\DecValTok{1}\NormalTok{)}
\NormalTok{    plot_data <-}\StringTok{ }\NormalTok{tmp_var}
  \ControlFlowTok{if}\NormalTok{(ii }\OperatorTok{>}\StringTok{ }\DecValTok{1}\NormalTok{)}
\NormalTok{    plot_data <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(plot_data, tmp_var)}
  \KeywordTok{rm}\NormalTok{(tmp_var)}
\NormalTok{\}}

\CommentTok{# To get the average value for a set of strata, weight the val by the area: (slow...)}
\NormalTok{mn_NEBS_season <-}\StringTok{ }\KeywordTok{getAVGnSUM}\NormalTok{(}
  \DataTypeTok{bysim =}\NormalTok{ T,}
  \DataTypeTok{strataIN =}\NormalTok{ NEBS_strata,}
  \DataTypeTok{dataIN =}\NormalTok{ plot_data,}
  \DataTypeTok{tblock=}\KeywordTok{c}\NormalTok{(}\StringTok{"yr"}\NormalTok{,}\StringTok{"season"}\NormalTok{))}
\NormalTok{mn_NEBS_season}\OperatorTok{$}\NormalTok{basin =}\StringTok{ "NEBS"}
...
\NormalTok{plot_data_}\DecValTok{2}\NormalTok{      <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(mn_NEBS_season,mn_SEBS_season)}

\CommentTok{# plot Fall values:}
\NormalTok{p6v2 <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ plot_data_}\DecValTok{2}\OperatorTok{%>%}\KeywordTok{filter}\NormalTok{(season}\OperatorTok{==}\StringTok{"Fall"}\NormalTok{) ) }\OperatorTok{+}\StringTok{ }
\StringTok{  }\KeywordTok{geom_line}\NormalTok{(   }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ yr,}\DataTypeTok{y =}\NormalTok{ mn_val,}\DataTypeTok{color=}\NormalTok{sim),}\DataTypeTok{alpha=}\NormalTok{.}\DecValTok{8}\NormalTok{)}\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_smooth}\NormalTok{( }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ yr,}\DataTypeTok{y =}\NormalTok{ mn_val,}\DataTypeTok{color=}\NormalTok{sim),}
               \DataTypeTok{formula =}\NormalTok{ y }\OperatorTok{~}\StringTok{ }\NormalTok{x, }\DataTypeTok{se =}\NormalTok{ T)}\OperatorTok{+}
\StringTok{  }\KeywordTok{facet_grid}\NormalTok{(basin}\OperatorTok{~}\NormalTok{.)}\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_color_viridis_d}\NormalTok{(}\DataTypeTok{begin=}\NormalTok{.}\DecValTok{2}\NormalTok{,}\DataTypeTok{end=}\NormalTok{.}\DecValTok{8}\NormalTok{)}\OperatorTok{+}
\StringTok{  }\KeywordTok{ylab}\NormalTok{(plot_data_}\DecValTok{2}\OperatorTok{$}\NormalTok{units[}\DecValTok{1}\NormalTok{])}\OperatorTok{+}
\StringTok{  }\KeywordTok{ggtitle}\NormalTok{( }\KeywordTok{paste}\NormalTok{(sim,}\StringTok{"Fall"}\NormalTok{,mn_NEBS_season}\OperatorTok{$}\NormalTok{var[}\DecValTok{1}\NormalTok{]))}\OperatorTok{+}
\StringTok{  }\KeywordTok{theme_minimal}\NormalTok{()}
\NormalTok{p6v2}
...
\ControlFlowTok{if}\NormalTok{(update.figs)  }
  \KeywordTok{ggsave}\NormalTok{(}\DataTypeTok{file=}\KeywordTok{file.path}\NormalTok{(main,}\StringTok{"Figs/Fall_large_Zoop_bySSP.jpg"}\NormalTok{),}\DataTypeTok{width=}\DecValTok{8}\NormalTok{,}\DataTypeTok{height=}\DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{    p9}
    ...
    \ControlFlowTok{if}\NormalTok{(update.figs)  }
      \KeywordTok{ggsave}\NormalTok{(}\DataTypeTok{file=}\KeywordTok{file.path}\NormalTok{(main,}\StringTok{"Figs/sub_grid_mn_BT_Aug1.jpg"}\NormalTok{),}\DataTypeTok{width=}\DecValTok{8}\NormalTok{,}\DataTypeTok{height=}\DecValTok{6}\NormalTok{)}
